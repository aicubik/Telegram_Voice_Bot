...
def compare_pollinations():
    ...
    models = ["flux", "z-image-turbo", "nanobanana-pro"]
    
    # Create directory for results if not exists
    os.makedirs("scratch/testing/comparison", exist_ok=True)
    
    print(f"--- Pollinations Model Comparison ---")
    print(f"Prompt: {prompt}\n")
    
    for model in models:
        url = f"https://image.pollinations.ai/prompt/{encoded_prompt}?model={model}&seed={seed}&width=1024&height=1024&nologo=true"
